...
void main() {
  group('Matrix adjacent negative number parsing', () {
    test('parses matrix(0-1 1 0...) format', () {
      ...
      print(
        "Test 1 (0-1): ${t1.length} transforms, values: ${t1.isNotEmpty ? t1[0].values : "none"}",
      );
    ...
    test('parses standard comma format', () {
      final t2 = SvgTransform.parse(
        "matrix(0, -1, 1, 0, 136.353074, 431.758938)",
      );
      print(
        "Test 2 (commas): ${t2.length} transforms, values: ${t2.isNotEmpty ? t2[0].values : "none"}",
      );
    ...
    test('parses matrix(0-.363742...) format', () {
      final t3 = SvgTransform.parse(
        "matrix(0-.363742 0.363742 0 93.814966 511.567068)",
      );
      print(
        "Test 3 (0-.363742): ${t3.length} transforms, values: ${t3.isNotEmpty ? t3[0].values : "none"}",
      );
  ...
  group('Text rendering with matrix transforms', () {
    ...
    testWidgets('text with multiple tspans and different letter-spacing', (
      tester,
    ) async {
      const svg = '''
    ...
    testWidgets('text with namespaced font-family falls back gracefully', (
      tester,
    ) async {
