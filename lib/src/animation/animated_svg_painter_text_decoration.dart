...
extension AnimatedSvgPainterTextDecorationExtension on AnimatedSvgPainter {
  ...
  void _drawParagraphWithEffects(
    ...
    final bounds = ui.Rect.fromLTWH(x, y, paragraph.maxIntrinsicWidth, paragraph.height).inflate(1.0);
  ...
  void _drawTextEmphasisMarks(ui.Canvas canvas, ui.Paragraph paragraph, double x, double y, String text, _ResolvedTextStyle style) {
    ...
    final emphasisColor = style.textEmphasisColor != null ? _resolveColorFromString(style.textEmphasisColor!) : style.color;
    final markFontSize = style.fontSize * 0.5;
    final markSpacing = style.fontSize * 0.2;

    final emphasisParagraphStyle = ui.ParagraphStyle(fontSize: markFontSize, textAlign: ui.TextAlign.center);
    final emphasisBuilder = ui.ParagraphBuilder(emphasisParagraphStyle);
    emphasisBuilder.pushStyle(ui.TextStyle(color: emphasisColor, fontSize: markFontSize));
    ...
    for (var i = 0; i < glyphs.length; i++) {
      ...
      final markY = isAbove ? y - markSpacing - emphasisParagraph.height : y + paragraph.height + markSpacing;
  ...
  String? _resolveEmphasisMarkCharacter(String style) {
    ...
    for (final part in parts) {
      switch (part) {
        case 'filled': isFilled = true; break;
        case 'open': isFilled = false; break;
        case 'dot': case 'circle': case 'double-circle': case 'triangle': case 'sesame': markType = part; break;
        default:
          if (part.startsWith("'") || part.startsWith('"')) return part.substring(1, part.length - 1);
          else if (part.isNotEmpty && !['none', 'filled', 'open'].contains(part)) return part;
      }
    }

    switch (markType ?? 'dot') {
      case 'dot': return isFilled ? '\u2022' : '\u25E6';
      case 'circle': return isFilled ? '\u25CF' : '\u25CB';
      case 'double-circle': return '\u25CE';
      case 'triangle': return isFilled ? '\u25B2' : '\u25B3';
      case 'sesame': return isFilled ? '\uFE45' : '\uFE46';
      default: return null;
    }
  }
  ...
  ui.Color _resolveColorFromString(String colorStr) {
    ...
    const colorMap = <String, int>{
      ...
      'gray': 0xFF808080, 'grey': 0xFF808080, 'orange': 0xFFFFA500, 'purple': 0xFF800080,
      'pink': 0xFFFFC0CB, 'brown': 0xFFA52A2A,
    };

    if (colorMap.containsKey(normalized)) return ui.Color(colorMap[normalized]!);
    ...
    if (normalized.startsWith('#')) {
      ...
      if (hex.length == 6) {
        ...
        final r = hex[0]; final g = hex[1]; final b = hex[2];
  ...
  String _applyTextTransform(String text, String textTransform) {
    ...
    switch (textTransform) {
      ...
      case 'capitalize': return _capitalizeWords(text);
      case 'full-width': return _toFullWidth(text);
      default: return text;
    }
  }
  ...
  String _toFullWidth(String text) {
    ...
    for (final rune in text.runes) {
      if (rune >= 0x21 && rune <= 0x7E) buffer.writeCharCode(rune + 0xFEE0);
      else if (rune == 0x20) buffer.writeCharCode(0x3000);
  ...
  ui.TextDecorationStyle _mapDecorationStyle(String? value) {
    switch (value) {
      ...
      case 'solid': default: return ui.TextDecorationStyle.solid;
    }
  }
  ...
  ui.Color? _tryParseColor(String token) {
    ...
    const colorMap = <String, int>{
      'black': 0xFF000000, 'white': 0xFFFFFFFF, 'red': 0xFFFF0000, 'green': 0xFF008000,
      'blue': 0xFF0000FF, 'gray': 0xFF808080, 'grey': 0xFF808080, 'transparent': 0x00000000,
    };
    if (colorMap.containsKey(normalized)) return ui.Color(colorMap[normalized]!);
