...
// Resolution strategy:
//   - Tests (FLUTTER_TEST=true): allow an env override (LIBQUICKJSC_TEST_PATH)
//     for CI; otherwise locate the dylib we built under the package's own
//     native/build directory. The fallback path is resolved relative to a
//     pub-cache install or a path-dep — Dart resolves `package:full_svg_flutter_js`
//     URIs at runtime so we don't need to hardcode the workspace path.
//   - App mode (plugin loaded): on Apple platforms symbols live in the host
//     process (CocoaPods links the bridge into the .app); on Linux/Windows the
//     plugin CMake places the shared library next to the executable; on
//     Android the bridge ships as libquickjs_c_bridge_plugin.so inside the APK.
DynamicLibrary _openTestLib() {
  ...
  // Search well-known locations relative to Directory.current (which is the
  // project root during `flutter test`). The fork is always consumed as a
  // path dep so the build artifact has a predictable location.
  final candidates = <String>[
    'packages/full_svg_flutter_js/native/build/$libName',
    '../packages/full_svg_flutter_js/native/build/$libName',
  ...
  // Last-resort: fall through to a system loader path.
...
final DynamicLibrary _qjsLib = Platform.environment['FLUTTER_TEST'] == 'true'
    ? _openTestLib()
    : (Platform.isWindows
        ? DynamicLibrary.open('quickjs_c_bridge_plugin.dll')
        : (Platform.isLinux
