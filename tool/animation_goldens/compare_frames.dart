...
class SvgReport {
  SvgReport({required this.svg, required this.frames, required this.results});
  ...
  double get worstSimilarity {
    if (results.isEmpty) return 0.0;
    return results.map((r) => r.similarity).reduce((a, b) => a < b ? a : b);
  }
...
List<String> _discoverComparableSvgs() {
  ...
  final browserSvgs = browserDir
      .listSync()
      .whereType<Directory>()
      .map((d) => d.path.split('/').last)
      .toSet();

  final flutterSvgs = flutterDir
      .listSync()
      .whereType<Directory>()
      .map((d) => d.path.split('/').last)
      .toSet();
...
void main() {
  ...
  test(
    'Compare animation frames and generate reports',
    () async {
      ...
      if (allReports.isNotEmpty) {
        final totalFrames = allReports.fold<int>(0, (s, r) => s + r.frames);
        final overallAvg =
            allReports.map((r) => r.averageSimilarity).reduce((a, b) => a + b) /
            allReports.length;
        final above95 = allReports
            .where((r) => r.averageSimilarity >= 0.95)
            .length;
        final below90 = allReports
            .where((r) => r.averageSimilarity < 0.90)
            .length;
